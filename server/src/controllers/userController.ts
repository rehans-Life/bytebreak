...
export const userSubmissionsCount: RequestHandler = catchAsync(
  async (req, res, next) => {
...
export const userCalender: RequestHandler = catchAsync(
  async (req, res, next) => {
    const username = req.params.username
    ...
    const activeYears = Array.from(
      new Array(currYear - createdYear + 1),
      (_, i: number) => i + createdYear,
    )
